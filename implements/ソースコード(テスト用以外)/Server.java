/* version 12
 * 対戦相手のマッチング待ちであるクライアントの識別番号が初期化されるのは
 * quit()を呼び出したクライアントがログイン済かつ対戦相手がいない場合のみとなるように修正
 */
/* version 11
 * ログイン失敗時にそのクライアントに対するReceiverが閉じられるよう修正
 * ログイン失敗時のquit()では対戦相手のマッチング待ちであるクライアントの識別番号に変更を加えないよう修正
 */
/* version 10
 * 先手後手情報の送信時にプレイや名をコロンで区切って同時に送る処理を追加
 */
/* version 9
 * printStatus()の表示内容を変更
 */
/* version 8
 * クライアントの最大接続数を超えている場合に
 * "room is max"を送信する処理を追加(300行付近)
 */
/* version 7
 * サーバプログラムの標準入力に"printstatus"を入力すると
 * クライアントの接続状況を表示する機能を追加
 */

/* version 6
 * getLoginInfoをプレイヤ名にコロンが含まれる場合を扱えるように修正
 * printStatus()が2回表示されるのを1回の表示に修正, また表示内容にプレイヤ名を追加
 */
// version 5：クライアント接続の切断処理を修正
