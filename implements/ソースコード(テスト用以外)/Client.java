//追加：回転途中のアイコン, 1159行目のturnsquare()(changesquare()から変更), 1218行目のturnanimationThread()(アニメーション描画用スレッド)
//追加：1483行目からと1572行目から, 及び1874行目からと1930行目からをアニメーション描画用の処理に変更, 1631行目にアニメーション制御用のスレッドを追加
//コメントアウト：repaintallBoard() の石描画部分
//v7変更：アニメーション速度アップ、一刻千金モードで相手がパスをした際に自分の石のうち一番金額の低い石を1つ増やすように変更(パスされた側が最後に石が足りなくなるのを防ぐため)
//v7バグ修正：最初の黒の手番の時に白の側の盤面にも石を置ける場所が表示されるのを修正、対局が始まった後にまた対局開始のメッセージが表示されるのを修正、一刻千金モードの最後のスコア計算に金額が反映されていないのを修正(getScore()いらないかも)
//v7バグ修正：対局終了後にもボタンが押せる→とりあえず対局終了時全てのボタンの反応を無効化するよう設定、一刻千金モードで左端の列の石をひっくり返す処理ができていなかったのを修正
//v8追加：1733行目にタイマーの時間を設定するsettime()を追加。1901行目1958行目でメッセージ受取時に相手のタイマーに合わせるよう追加。1485行目1571行目で石を置いた時自分のタイマーの1秒未満の値を切り上げるよう追加(相手とのズレを修正するため)。
//v8変更：相手のタイマーを開始するタイミングを石をひっくり返すアニメーションが全て終わった後に変更
//v9変更：1930行目と1989行目の送信するメッセージの内容を変更(パスの場合も書式を通常の場合に合わせる)
//v10変更：パスの時送信するメッセージを一刻千金モード用に変更、2030行目と2023行目がノーマルモードの勝敗処理になっていたので一刻千金モード用に変更、チャット画面と対局画面の初期位置をウィンドウ中央に変更
//v10変更：相手が対局画面を開くまで操作を受け付けないよう変更(相手が画面表示する前に最初の一手を打つと不具合が生じるため)、相手が対局画面を開く前にも正しい残り時間を表示するよう変更
//v10変更：チャット画面で時間決定確認結果が複数表示されないように変更(同じ色の側で複数対局画面が開くのを防止するため)、チャット機能を制限時間決定確認画面が開いて以降は新たなメッセージを送れないよう変更
//v10変更：制限時間確認画面及び制限時間決定確認画面はウィンドウの閉じるボタンでは閉じないように変更(チャット画面の仕様変更に伴い起こる可能性のある、それらを閉じるボタンで閉じた際の不具合を防止するため)
//v11変更：チャット画面のメッセージ表示枠(左上の枠)を編集不可に変更(454行目)
//v11変更：一刻千金モードで石の数が0個のボタンはカーソルを合わせても枠を表示せず、色をグレーにするように変更(1590～1594行目、1649～1655行目)
//v12変更：パスの時、確認ダイアログを表示する前に一旦"stopmytimer"というメッセージを送り、相手側の自分のタイマーを止めてもらうように変更
//v12変更：一刻千金モードの石選択ボタンについて、石を置いた後もその石を選択したままの状態(次の手の初期選択)にするよう変更
//v12変更：最初の一手では一番価格の低い石を初期選択としておくように変更、石を置き、選択している石がなくなった場合はその時点で持っている石の中で一番価格の低い石を初期選択に設定するよう変更
//v13変更：制限時間を過ぎた際の処理を追加(gameTimerクラスにif文で追加)1804行目
//v13変更：制限時間を過ぎた際の画面表示用メソッドtimeResignDisp追加1490行目
//v14修正：サーバからの切断処理が正しく動く様に修正、チャット及び対局の途中で相手のサーバへの接続が切断された場合にタイトルに戻るよう変更
//v14変更：サーバ接続切断タイミングを、対局終了画面で"退室する"ボタンを押したときに変更
//v15変更：対局終了画面表示後は相手のサーバ接続切断通知を表示しないように変更、相手の手番で終わった場合と同様に自分の手番で終わった場合も石をひっくり返すアニメーションが全て終わった後に対局終了判定をするように変更
//v16変更：チャット画面で、コロンが含まれるメッセージが見やすくなるよう微調整(676行目)
//v16変更：チャット画面で、コロンが含まれるメッセージが正しく受信されないのを修正(2038～2048行目)
//v17修正：サーバ同時接続可能人数が超過していた場合の処理を修正
//v18追加：BGM及び効果音を鳴らす機能を追加
//v19変更：前に鳴っていたBGMを止める処理の方法を変更
//v20追加：音を流すかを切り替えるボタンをタイトル画面に追加(77行目、110 154～161行目、307行目、331～355行目、377行目、2388行目、2416行目)
//v21修正：音を流れないようにしていた場合にクライアントを再起動した際、音が流れる状態になってしまうバグを修正(78行目、161～168行目)
//v22変更：一刻千金モードで対戦相手のタイマーの不具合を修正(予防)と、
//         パスの時は相手がパスかどうか判断し、パスの時は相手のタイマーを動かさないように変更(修正：1881～1929行目、コメントアウト：1899,1926行目)
//v22変更：パスの時の処理の変更に伴い、"stopmytimer"の送信処理をコメントアウト(コメントアウト：2216, 2289行目)
//v22変更：パスの時も、相手から受け取ったタイマーに合わせるように調整(コメントアウト：2189, 2252行目、コメント解除：2185, 2248行目)
//v22変更：対局時のBGMを、双方が対局画面を表示してから流し始めるように変更(対局開始をわかりやすくするため)(追加：1022, 1244行目、コメントアウト：776行目)
//v23変更：BGMと同じく効果音も1度鳴らすごとにClipを閉じるように変更(メモリ枯渇防止のため)
//v24修正：対戦相手が見つからなかった場合に部屋が満員で入れなかったというメッセージが出てしまっていたので修正
//v25変更：マッチング中、チャット中、対局中にサーバとの接続が切断された際はサーバ接続が切断されたというメッセージを表示してタイトルに戻るよう変更
//v25変更：マッチングに失敗した際、"OK"ボタンを押してからタイトル画面の各ボタンを押せるようになるよう変更
//         ("OK"ボタンを押さずともタイトル画面のボタンが戻っており、マッチング失敗の画面をずっと残したままにできてしまっていたため。予期せぬ動作を防止)
//v25修正：マッチングに失敗した際、マッチング失敗の画面を表示する前にサーバ接続を切断するよう修正(マッチング失敗の画面表示中に相手がログインすると対局が始まってしまうため)
//v25修正：ログインの時のサーバ接続失敗後やマッチング失敗後の処理が上手くいっておらず、タイトル画面に戻った後またログイン・マッチングしようとすると動かなくなっていたのを修正
//v25修正：対局画面表示後もchfがnullとなっていなかったので、対局画面表示直前にchfにnullを代入するよう修正
//         (相手やサーバの接続切断時、チャット画面が開いているかの判定の際chfがnullでないかを判定する処理を入れているため)
//v26修正：起動時にはchfにはnullを代入し、チャット画面を起動した際にChatJFrameオブジェクトを生成するよう変更(整合性をとるため)
//v26修正：タイトル画面関連のウィンドウをモードレス表示からモーダル表示に変更
//v26修正：各警告ダイアログのタイトルを変更
//v26修正：全てのcatchブロックで、printStackTrace()を呼ぶように変更(デバッグ用)
//v27変更：connectServerFailedDisp()を呼び出さないようになっていたのでconnectServerFailedDisp()をコメントアウトするように変更(479～506行目)
//v27修正：サーバ接続失敗時に"OK"ボタンを押してもタイトル画面のボタンが有効にならない不具合を修正(2113行目)
//v27修正：サーバ接続失敗直後にsendMessage()を実行してエラーが出ていたのを修正(447～449行目)
//v28追加：残り時間が1分未満となった際に残り時間の表示文字を赤くし、専用のBGMを流すよう追加
//v28追加：対局終了画面の表示時に勝利、敗北、引分に応じたBGMを流すよう追加
//v28追加：効果音として、汎用ボタン押下音、ボタンキャンセル音、エラーダイアログ表示音、石の種類決定ボタン押下音、相手の投了の通知音を追加
//v28変更：効果音を鳴らす度に前のクリップを閉じているとたまに重くなることがあったため、
//         titleDisp()実行時に全ての効果音のクリップを開き、次にまたtitleDisp()が呼び出されるまでは開きっぱなしとするように変更
//v28変更：パスの時と相手が投了した時のメッセージダイアログの表示先を対局画面に変更(対局画面の中央に表示される)
//v29変更：soundEffect()の記述を簡略化、及びsoundtitleの配列はプログラム起動時のみに作成するよう変更
//v30変更：制限時間切れの時、まれに制限時間切れじゃないほうの「相手の」タイマーが0になっていないことがあるので調整する処理を追加。
//		   その際に送るメッセージをtimeoverに変更
//v31変更：soundtitleの配列をsoundEffect("open")が実行される度に作成するよう変更
//v32変更：相手のプレイヤ名を受け取り保持するように変更、チャットメッセージ及びチャット画面のタイトル、対局画面のタイトルに自分と相手のプレイヤ名を表示するよう変更
//v33変更：プレイヤ名とチャットメッセージの末尾がコロンだと受信メッセージに反映されないのを修正
//v34変更：プレイヤ名とチャットメッセージの末尾にコロンが複数回続いた場合も正しく受信できるように修正
//v35変更：受け取った残り時間と自分の持ってる相手の残り時間の差が5秒より大きい場合に不正の可能性のあるエラーとして処理する
//         メッセージとして"timeerror"を追加
//　　　　その場合の画面表示は「引き分け」残り時間は--:--に成るようにresultDispを一部変更
//v36変更：片方が"退室する"ボタンを押した時点でもう片方の通信も切断するようになっているので、後から"退室する"ボタンが押された際は通信切断処理を飛ばすよう変更
//v37変更：追加したBGM名、効果音名を反映
//v38変更：画面の高さを変更(2カ所：対局画面、プレイヤ名入力)
//v39変更：時間決定確認結果画面の表示で、文字とボタンがうまく表示されないバグを修正
//v40変更：チャット関連の他の画面についても、v39と同様の微調整
//v41修正：一度音をオフにしてからオンにすると効果音が流れなくなるバグを修正(462行目)
//v42変更：messageという変数名が紛らわしかったのでtitle_messageに変更
//v42変更：title_messageによる受信メッセージの保持はマッチング後からは行わないよう変更(マッチング後はtitle_messageを使わないため)
//v43追加：BGMと音量を調節するスライダーを追加(タイトル画面、チャット画面、対局画面)
//v43追加：音量を再設定するメソッドsetVolume()の追加
//v43変更：スライダーの追加による、チャット画面と対局画面のレイアウトの微調整
//v44修正：タイトル画面でSOUND:OFFにしてチャット画面、対局画面に移っても音量を調節できるように修正
//v44修正：上の修正に伴いメソッドbgmも変更(2809行目)
//v45追加：音量を調節するスライダーの上に音量を表示するラベルを追加
//v45変更：タイトル画面(再起動時)、チャット画面、対局画面について、それぞれ直前まで表示されていたウィンドウの中央を最初の表示位置となるように変更
//v45変更：時間決定確認画面か時間決定確認結果画面が表示された後はチャットメッセージ入力及び希望持ち時間の入力をできないように変更
//v45修正：チャット画面で相手が時間を決めた際、時間決定確認結果画面の表示が勝手にチャット画面の後ろに移動していたのを修正
//v45修正：投了確認画面を表示した状態で時間切れとなって勝敗が確定した後投了を行った際は投了の処理を行わないように修正
//v46変更：タイトル画面でも、音量0だと音量のラベルがグレーになるように変更(253～258行目、280～285行目)
//v47追加：Clientクラスの属性に一部コメントを追加
//v47変更：タイトル画面でSOUND:OFFにした場合はチャット画面及び対局画面で音量を調節するスライダーを表示しないように変更
//v47変更：SOUND:OFFの場合チャット画面及び対局画面ではBGMを流さないのでbgm()でplaysoundによる判定を行うように変更
//v47変更：現状使われていない変数をコメントアウト
//v47修正：チャット画面で相手の接続が切れた場合のメッセージ表示位置が画面中央になっていたので、チャット画面の中央に表示するよう修正
//v48変更：黒のプレイヤの対局画面で、石を置ける場所を、対局を開始してから表示するように変更
//v49修正：マッチング相手が見つからなかった場合にタイトル画面のボタンが押せないままとなっていたので修正
//v49変更：Clientクラスのインスタンスを再起動した際、既に効果音ファイルを開いている場合はsoundEffect("open")を実行しないように変更
//v49変更：退室ボタンを押した際のsoundEffect()のコメントアウトを解除(soundEffect()の実行直後に再起動する際も効果音を鳴らせるようになったため)
//v50変更：getStoneAmount()をgetAmount()に変更(処理が同じであるためgetStoneAmount()を削除)(1356～1357行目、1770～1771行目)
//v51変更：タイトル画面のウィンドウ名の"Othello"及び対戦モード選択用ボタンの"一刻千金"を"一石千金"に変更
//v51変更：タイトル画面の対戦モード選択用ボタンに画像を使わないよう変更
//v52変更：MacOSでボタンの色が正しく表示されていなかったので、各ボタンの背景色設定時にsetOpaque(true)を行うよう変更
//v53変更：v52で正しく修正されなかったのでsetBorderPainted(false)も行うよう変更
//v54変更：v53の更新部分を消去、Look&Feelの指定をtitleDisp内で行なうように変更(いくつかインポート、217～221行目を追加)
//v55変更：チャット画面の同期処理を対局画面の同期処理と同じ形式に変更
//v56追加：チャット画面にチャットの使い方及び一石千金モードの説明を追加
//v56変更：持ち時間決定について、希望持ち時間を相手に送信し、相手がその持ち時間を承認した場合に決定となるよう変更
//v57修正：相手の時間決定承認待ち画面における相手に送信した時間の表記が正しくなかったので修正
//v57修正：チャット画面における一石千金モードの説明が一部実際の仕様と異なっていたので正しい説明に修正
//v58修正：相手が持ち時間を承認した際に相手の時間決定承認待ち画面を閉じる処理が2重に行われていたのでreceiveMessage()内では行わないように修正
//v59修正：相手が持ち時間承認を拒否した際に相手の時間決定承認待ち画面が消えなくなっていたので修正
//v60修正：相手の時間決定承認待ち画面が表示されているとき相手もしくはサーバの接続が切れた場合に相手の時間決定承認待ち画面が表示されたままとなっていたので修正
//v61修正：複数回ログインに失敗(同じ名前の入力)した場合の挙動が不安定だったので、ログイン画面を開く際に既にログイン画面が開いているか確認するよう修正
//v62修正：2度目以降のログインの際Receiver_titleが複数起動した状態となる場合があったので、2度目以降のログインでは前のReceiver_titleが終了するまで待つよう修正
//v62変更：v62での修正に伴いv61の修正で追加した部分が不要となったので削除
